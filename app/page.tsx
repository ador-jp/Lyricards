    ...
    <div className="mx-auto grid max-w-7xl gap-6 px-5 py-7 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="space-y-6">
        ...
        <div className="panel"><div className="step"><span>2</span><div><h2>歌詞を貼り付ける</h2><p>利用権のある歌詞だけを入力してください。歌詞本文は保存しません。</p></div></div>
          {selected && <><div className="mt-4 flex items-center gap-3 rounded-xl bg-accent/60 p-3"><img className="size-11 rounded-lg" src={selected.artworkUrl100} alt=""/><div className="min-w-0 flex-1"><strong className="block truncate">{selected.trackName}</strong><span className="text-sm text-muted-foreground">{selected.artistName}</span></div><a aria-label="曲のページを開く" href={selected.trackViewUrl} target="_blank" rel="noreferrer"><ExternalLink size={18}/></a></div><div className="mt-3 flex flex-wrap gap-2"><a className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground" href={`https://www.musixmatch.com/search/${encodeURIComponent(`${selected.trackName} ${selected.artistName}`)}`} target="_blank" rel="noreferrer">Musixmatchで確認 <ExternalLink size={14}/></a><a className="inline-flex items-center gap-2 rounded-md border px-4 py-2 text-sm font-medium text-primary" href={`https://www.uta-net.com/search/?Keyword=${encodeURIComponent(`${selected.trackName} ${selected.artistName}`)}`} target="_blank" rel="noreferrer">歌ネットで確認 <ExternalLink size={14}/></a></div><p className="mt-2 text-xs text-muted-foreground">正規サイトで歌詞を確認し、個人学習の範囲で下欄へ貼り付けてください。</p></>}
